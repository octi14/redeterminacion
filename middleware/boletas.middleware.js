...
module.exports = async function requireBoletas(req, res, next) {
  try {
    ...
    if (!["admin", "master", "true", "boletas"].includes(legacyRole)) {
      return res.status(403).json({ message: "Esta operación requiere permisos de boletas." });
    }
