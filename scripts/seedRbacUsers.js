...
async function run() {
  ...
  const existingRoles = await RbacRole.find({ key: { $in: roleKeys }, active: true }).lean();
  ...
  if (missingRoles.length) {
    throw new Error(`Faltan roles: ${missingRoles.join(", ")}`);
  }
  ...
  console.log(`Usuarios RBAC creados/actualizados: ${users.length}`);
